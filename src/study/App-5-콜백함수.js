...
import './App.css';
import Post from '../compoent/Post'; // .js 생략가능
...
function App(){
  ...
  //좋아요 증가
  // const increaseGood = (id) =>{
  //   let copy = [...posts]
  //   // 매개변수로 받아온 id와 같은 객체 => 그 객체의 인덱스 번호를 찾는다
  //   const index = copy.findIndex(el => el.id === id) 
  //   copy[index].good += (copy[index].good<50) ? 1 :0
  //   setPosts(copy)
  // }
  ...
  // 타이틀 변경
  // const editPostTitle = (id)=>{
  //   // 배열 값 바꿀 때
  //   let copy = [...posts]; // 배열 스프레드복사로 값을 카피
  //   const index = copy.findIndex(el => el.id === id)
  //   copy[index].title = '홍대 맛집' // 복사된 배열의 객체 값을 바꿈
  //   setPosts(copy) // 변경된 배열로 통째로 값을 변경
  // }


  // 아래 방식은 데이터의 값이 변동 될때마다 함수를 다시 재랜더링함
  const editPostTitle = useCallback((id)=>{
